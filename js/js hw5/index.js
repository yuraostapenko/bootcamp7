...
const getUsersByEyeColor =(el, eyeColor) => {return users.filter(el => el.eyeColor === eyeColor)};
console.log(getUsersByEyeColor(users, 'blue')); // [объект Moore Hensley, объект Sharlene Bush, объект Carey Barr]



const getUsersByGender = (el, gender) => {return users.filter(el => el.gender === gender)};
console.log(getUsersByGender(users, 'male')); // [ 'Moore Hensley', 'Ross Vazquez', 'Carey Barr', 'Blackburn Dotson' ]



const getInactiveUsers = () => {return users.filter(el => el.isActive === false)};
 ...
 */
const getUserByEmail = (arr, email) => {return users.find(el => el.email === email)};
...
//  */
const getUsersWithAge = (arr, min, max) => {return users.filter(el => el.age > min && el.age < max)};
